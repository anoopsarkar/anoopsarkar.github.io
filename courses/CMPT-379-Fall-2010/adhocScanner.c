...
{    
  ...
  while (ScanOneToken(stdin, &token) != T_END) {
    ...
    switch(token.type) {
    case T_WHILE: printf("%s","T_WHILE"); case T_IF: printf("%s","T_IF"); 
    case T_RETURN: printf("%s","T_RETURN"); case T_IDENTIFIER: printf("%s","T_IDENTIFIER"); 
      printf(" value:\'%s\'", token.val.stringValue);
      break;
    case T_STRING:
      printf("T_STRING value:\"%s\"", token.val.stringValue);
      break;
    case T_INTEGER:
      printf("T_INTEGER value:%d", token.val.intValue);
      break;
    case T_DOUBLE:
      printf("T_DOUBLE value:%lf", token.val.doubleValue);
      break;
    default:
      if (token.type < 127) {
        switch(token.type) {
